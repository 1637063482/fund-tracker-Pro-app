...
## 五、AI System Prompt 架构

```
第一层: 防幻觉协议（净值唯一通道、T+1 妥协、资金交收）
第二层: 活体战略记忆（双层结构、动态验证、反讨好）
第三层: 专属技能库（22 工具 × 6 分类，跨工具调用铁律）
第四层: 双核打分系统（权益4因子 + 固收2因子 + 动量修正 + 滞回锁定 + 全局否决）
第五层: 记忆库巡检洗盘（前置打分 → 战术拦截 → 防污染 → 自检回顾）
```
