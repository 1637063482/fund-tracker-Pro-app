...
### 修复
- **状态栏云端连接状态一直显示"连接中"**：`isDbConnected` 状态从未被设为 `true`。新增 `useEffect` 在用户认证成功时标记数据库已连接，状态栏正确显示绿色圆点 + "云端已同步"

### 变更
- **F1 打分架构重构**：原单一 F1(35分) → F1a 上证赔率(20分) + F1b 双创风格校验(15分)，双指数并行评估后相加
- **双创数据常态化拉取**：打分前置清单新增 a3/a4/a5 步骤，创业板日K×60+周K×60 始终拉取，科创50在走势分歧时补拉
- **前置清单步骤重排**：`get_market_microstructure` 从步骤 g 提升为步骤 0（最先调用），作为 F3 微观结构熔断判定依据
- **全局否决阈值调整**：从 `F1<7` 改为 `F1a<4`（仅看上证系统性风险），战术警戒从 `F1=7-12` 改为 `F1≤12`
- **F4 Default 兜底补全**：跨资产确认新增兜底分支"否则→中性/缺数据 5-7"
- **评分输出格式更新**：单行格式改为 `F1:28/35(F1a:16/20+F1b:12/15) F2:18/25...`，巡检报告公示五因子明细（F1a/F1b/F2/F3/F4）
- **F2 档位微调**：各档位分数区间加宽，减少死区间
- **MECE 对齐**：F1a 档位边界重叠≤1分，消除相邻档位间的分数真空
