// 基金持仓表格组件：投资组合/清仓历史双 Tab 表格，展示持仓市值、占比、盈亏、XIRR 等列，支持排序与操作
...
export const FundTable = ({
  ...
  <div className="apple-card overflow-hidden flex flex-col transition-colors duration-500">
    <div className="flex justify-between items-end border-b dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/20 px-3 sm:px-5 pt-4 sm:pt-5 relative overflow-x-auto no-scrollbar">
      ...
      <button type="button" onClick={(e) => { if (onCaptureRect) onCaptureRect(e.currentTarget.getBoundingClientRect()); setEditingFundId('new'); }} className="bg-blue-500 hover:bg-blue-600 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-sm font-medium flex items-center shrink-0 transition-all shadow-apple-sm active:scale-[0.97] mb-2 group">
    ...
    <div className="overflow-x-auto relative">
      <table className="w-full text-center min-w-[720px]">
        <thead>
          <tr className="bg-slate-50 dark:bg-slate-900/80 text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b dark:border-slate-700 select-none">
        ...
        <tbody className="divide-y dark:divide-slate-700 text-sm sm:text-base xl:text-lg relative">
          ...
          {sortedFunds.map((fund, fIndex) => (
            <tr key={fund.id} style={{animationDelay: `${fIndex * 50}ms`}} className={`group table-row-lift transition-all duration-200 animate-fade-in-up cursor-pointer ${fundTab === 'active' ? 'hover:bg-white dark:hover:bg-slate-800 hover:shadow-md hover:z-10 relative border-b border-slate-100 dark:border-slate-700/50 border-l-2 border-l-transparent hover:border-l-blue-400 dark:hover:border-l-blue-500' : 'bg-slate-50/50 dark:bg-slate-900/30 opacity-80 hover:bg-white dark:hover:bg-slate-800 hover:shadow-md hover:z-10 relative border-l-2 border-l-transparent hover:border-l-amber-400 dark:hover:border-l-amber-500'}`}>
                ...
                <div className="flex flex-col">
                  <div className="flex items-center">
                    ...
                    {fund.mode === 'auto' && fundTab === 'active' ? (
                      <div className="flex items-center">
                        <button type="button" onClick={(e) => { if (onCaptureRect) onCaptureRect(e.currentTarget.getBoundingClientRect()); handleViewProfile(fund.fundCode); }} className="text-slate-800 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300 font-bold whitespace-nowrap underline decoration-dashed decoration-blue-200 dark:decoration-blue-800 underline-offset-4">
                          {fund.name}
                        </button>
                  ...
                  {fund.mode === 'auto' && fundTab === 'active' && (
                    <div className="text-[11px] text-slate-500 mt-2 flex items-center gap-x-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/50 pl-1.5 pr-2 py-1 rounded-[0.625rem] w-fit shadow-sm whitespace-nowrap">
                      <button type="button" onClick={(e) => { e.stopPropagation(); fetchFundNavManually(fund.fundCode); }} className={`p-1 rounded-[0.625rem] hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-indigo-500 transition-all active:scale-90 ${fetchingNavCodes[fund.fundCode] ? 'animate-spin text-indigo-600' : 'hover:rotate-180 duration-500'}`} title="单点强力刷新净值">
                        <RefreshCcw size={12}/>
                      </button>
              ...
              <td className="p-4 sm:p-5 text-center whitespace-nowrap touch-visible-actions opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                <div className="flex justify-center items-center">
                  ...
                  <Tooltip content="永久删除">
                    <button type="button" onClick={(e) => { if (onCaptureRect) onCaptureRect(e.currentTarget.getBoundingClientRect()); handleDeleteFund(fund.id); }} className="text-slate-400 hover:text-red-500 mx-0.5 sm:mx-1 p-2 rounded-[0.625rem] hover:bg-red-50 dark:hover:bg-red-900/30 transition-all active:scale-[0.92] whitespace-nowrap touch-target">
                      <Trash2 size={18}/>
                    </button>
                  </Tooltip>
                </div>
