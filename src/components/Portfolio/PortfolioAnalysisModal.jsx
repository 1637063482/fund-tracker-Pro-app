...
export const PortfolioAnalysisModal = ({ portfolioStats, settings, onClose }) => {
  ...
  const handlePushToNtfy = async () => {
    ...
    try {
      ...
      const title = encodeURIComponent('💼 CIO 级全盘资产诊断报告');
      const tags = 'briefcase,sparkles';
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-white dark:bg-slate-900">
            {!aiReport && !aiLoading && !aiError && (
               <div className="text-center py-10 animate-in fade-in zoom-in">
                 ...
                 <h4 className="text-lg font-bold mb-2 text-slate-800 dark:text-white">准备好进行全盘上帝视角扫描了吗？</h4>
                 <p className="text-slate-500 dark:text-slate-400 text-sm mb-6 max-w-md mx-auto">
                    AI 将读取你当前全部持仓比重、盈亏分布，并实时连接搜索引擎检索全球宏观大局，为你出具专业的资产结构优化与调仓方案。
                 </p>
            ...
            {aiLoading && (
               <div className="flex flex-col items-center justify-center py-16 animate-pulse text-indigo-600 dark:text-indigo-400">
                 ...
                 <p className="font-medium text-sm">AI 正在调取全球宏观数据并透视您的资产负债表，请稍候...</p>
               </div>
            )}
               ...
               <div className="bg-indigo-50/50 dark:bg-slate-800 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-800/50 shadow-sm animate-in slide-in-from-bottom-4 duration-500 relative">
                 ...
                 <div className="mt-6 pt-4 border-t border-indigo-100 dark:border-indigo-900/30 flex justify-end">
                   <button 
                     ...
                     className={`px-5 py-2.5 rounded-xl text-sm font-bold flex items-center transition-all shadow-sm ${pushSuccess ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-indigo-600 hover:bg-indigo-700 text-white active:scale-95'}`}
                   >
                     {pushSuccess ? <><Check size={16} className="mr-1.5"/> 报告已飞送到手机</> : (isPushing ? <><RefreshCw size={16} className="mr-1.5 animate-spin"/> 正在发送...</> : <><Send size={16} className="mr-1.5"/> 一键将报告推送到手机</>)}
                   </button>
                 </div>
               </div>
            )}
