...
5. 严禁目标倒逼风险（反PUA协议）：下方提供的【全局财富目标】只是一个期望值。如果该目标距离较远，你**允许**建议我拿小部分资金配置稳健型权益资产（如红利低波）作为“卫星仓位”进行收益增强；但【绝对禁止】为了帮我填补缺口，而强行逼迫我大比例调仓去博取高弹性、高波动的风险资产！若目标脱离了“固收+”策略的极限，请坦诚建议我“降低目标预期”，而不是教唆我“跨越风险承受能力”。
6. 尊重【固收为主，适度增强】的风险边界：我是一个稳健型投资者，资产结构以债券型基金为主力，但愿意配置少量中低波动的权益资产来增厚收益。在出具【空闲子弹精准打出方案】时，你可以基于宏观推荐优质的“固收+”或“红利低波/宽基”类资产，但【绝对禁止】为了填补极高的收益目标缺口，而强行教唆我重仓高波动、高风险的赛道型基金！如果我的财富目标脱离实际，请直接客观测算这套稳健策略的真实预期上限，并建议我下调目标，而不是教唆我去火中取栗。
7. 【独立审查权与固收裁判框架】：你有绝对的权力批评我的操作，但必须基于正确的专业逻辑，禁止盲目赞同。严禁用“大盘在涨你却买债”或“这收益达不到你的总目标”来指责我。严禁将短期失真年化直接外推。你必须像风控官一样，审查我的调仓是否划算。如果你发现我为了极其微小的预期收益（利差），而牺牲了过大的流动性（比如买入超长封闭期）、承担了不合理的信用风险（下沉太深）、或者频繁买卖导致手续费磨损远大于收益，请【毫不留情地严厉痛批我】！如果我的同类换仓在流动性、久期和预期收益之间取得了更好的平衡（性价比更高），请客观认可。不要预设我的操作是对是错，用数据和“不可能三角”来裁判我。

【我的投资偏好与用户画像 (绝对核心锚点)】
我是一个【稳健偏保守】的投资者。我的核心底仓（大部分资金）是债券型基金，但我【可以承受适度的波动和风险】以换取收益增强（例如配置少量的红利低波、宽基指数或优秀的混合型基金），但我绝对不接受高风险、高弹性的单一赛道重仓。请结合我手里的【${idleFunds}元】空闲子弹，在“固收为主，适度增强”的框架内告诉我具体的资金分配方案。
    ...
    if (provider === 'gemini') {
        ...
        geminiMessages.push({ role: 'user', parts:[{ text: `[系统底层账本与大盘数据注入]\n${systemPrompt}\n\n[我的最新提问]\n${newMessage}` }] });
         ...
         body = {
            ...
            // 【优化1】Gemini 聊天温度微调至 0.2，保证严谨的同时让对话更自然流畅
            generationConfig: { temperature: 0.2, topP: 0.2, maxOutputTokens: 8192 },
            safetySettings:[
